...
const minvalueteste = process.env.MINVALUE || 7;
const maxvalueteste = process.env.MAXVALUE || 9;
//executa rolagem de um dado
function roll(nfaces = 6) {
  return Math.ceil(Math.random() * nfaces);
}
//executa teste de rolagem de dado
function testedice(modfic = 0, minvalue = 7, maxvalue = 9) {
  const t1 = roll(6);
  const t2 = roll(6);
  const testeroll = t1 + t2 + modfic;
  return {
    values: [t1, t2, modfic],
    result:
      testeroll > maxvalue
...
io.on("connection", function (socket) {
  ...
  socket.on("sendUser", async (data) => {
    ...
    if (data.typeuser === "narrador") {
      ...
      if (
        data.status.narrador &&
        data.status.user &&
        data.status.vaga &&
        data.status.playnumber
      ) {
    ...
    if (messages[data.narradorname]) {
      io.to(socket.id).emit("previousMessages", {
        username: data.username,
        hist: messages[data.narradorname],
      });
    }
    data.room = room;
    //console.log(data);
  ...
  socket.on("rollDice", (data) => {
    //formato exemplo {user:'user', modific:0, minvalue:7, maxvalue:9}
    let result = testedice(
      data.modific,
      data.minvalue ? data.minvalue : minvalueteste,
      data.maxvalue ? data.maxvalue : maxvalueteste
    );
